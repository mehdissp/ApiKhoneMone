...
public class MenuPermissionMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, AppDbContext _context)
    {
        ...
        var publicPaths = new[] { "/api/story/storyforsiteforuser","/api/story/storyforsite","/api/Auth/registerNewUserIndependent",
            "/api/realestatepage/getrealstatemap", "/api/realestatepage/getuserforsite",
            ...
            "/api/realestatepage/getcategorydtos", "/uploads/images",
            "/api/realestatepage/getrandomlastitemrealestates", "/api/auth/login", "/api/auth/register",
        ...
        var menu = await _context.Menus.FirstOrDefaultAsync(m => m.Url != null && paths.Equals(m.Url.ToLower()));
